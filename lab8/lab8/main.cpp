...
UINT g_CullingViewHeight = 720;
UINT g_CullingViewWidth = 1280;
float g_CamYaw = 0.0f;
    ...
    _In_ LPWSTR, _In_ int nCmdShow)
{
    ...
    RECT rc = { 0, 0, (LONG)g_CullingViewWidth, (LONG)g_CullingViewHeight };
    AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, FALSE);
    int winWidth = rc.right - rc.left;
    int winHeight = rc.bottom - rc.top;

    g_hCullingWindow = CreateWindowW(wc.lpszClassName, L"CG AS - HW 8: Frustum Culling",
    ...
    ShowWindow(g_hCullingWindow, nCmdShow);
    UpdateWindow(g_hCullingWindow);

    if (!InitializeDirect3D())
    {
        CleanupDirect3D();
        DestroyWindow(g_hCullingWindow);
        return (IsWindow(g_hCullingWindow) ? -1 : 0);
    }

    CreateBuffers();
    SetupColorBuffer(g_CullingViewWidth, g_CullingViewHeight);
    CreateGPUResources();
    CompileShaders();
    LoadTextures();
    LoadTextureArray();
    ...
    if (FAILED(hr)) { CleanupDirect3D(); DestroyWindow(g_hCullingWindow); return (IsWindow(g_hCullingWindow) ? -1 : 0); }
...
bool InitializeDirect3D()
{
    ...
    scd.BufferCount = 2;
    scd.BufferDesc.Width = g_CullingViewWidth;
    scd.BufferDesc.Height = g_CullingViewHeight;
    scd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    scd.BufferDesc.RefreshRate.Numerator = 60;
    scd.BufferDesc.RefreshRate.Denominator = 1;
    scd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
    scd.OutputWindow = g_hCullingWindow;
    ...
    depthDesc.Width = g_CullingViewWidth;
    depthDesc.Height = g_CullingViewHeight;
...
void CreateGPUResources()
{
    ...
    hr = g_pD3DDevice->CreateBuffer(&desc, nullptr, &g_pIndirectArgsUAV);
    assert(SUCCEEDED(hr));
    hr = g_pD3DDevice->CreateUnorderedAccessView(g_pIndirectArgsUAV, nullptr, &g_pIndirectArgsUAVView);
    assert(SUCCEEDED(hr));
    ...
    hr = g_pD3DDevice->CreateBuffer(&desc, nullptr, &g_pIndirectArgsDraw);
    assert(SUCCEEDED(hr));
    ...
    hr = g_pD3DDevice->CreateBuffer(&desc, nullptr, &g_pVisibleIdsStructured);
    assert(SUCCEEDED(hr));
    hr = g_pD3DDevice->CreateUnorderedAccessView(g_pVisibleIdsStructured, nullptr, &g_pVisibleIdsUAV);
    assert(SUCCEEDED(hr));
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
    srvDesc.Format = DXGI_FORMAT_UNKNOWN;
    srvDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;
    srvDesc.Buffer.NumElements = MAX_INSTANCES;
    hr = g_pD3DDevice->CreateShaderResourceView(g_pVisibleIdsStructured, &srvDesc, &g_pVisibleIdsSRV);
    assert(SUCCEEDED(hr));

    desc.ByteWidth = sizeof(CullParams);
    desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    desc.MiscFlags = 0;
    hr = g_pD3DDevice->CreateBuffer(&desc, nullptr, &g_pCullParamsCB);
    assert(SUCCEEDED(hr));

    desc.ByteWidth = sizeof(XMFLOAT4) * 6;
    hr = g_pD3DDevice->CreateBuffer(&desc, nullptr, &g_pFrustumPlanesCB);
    assert(SUCCEEDED(hr));
    ...
    qdesc.MiscFlags = 0;
    for (int i = 0; i < 10; ++i) {
        hr = g_pD3DDevice->CreateQuery(&qdesc, &g_pQueries[i]);
        assert(SUCCEEDED(hr));
    }
    ...
    g_pD3DContext->UpdateSubresource(g_pIndirectArgsUAV, 0, nullptr, &args, 0, 0);
}
...
void CompileShaders()
{
    ...
    HRESULT hr = D3DCompile(cullCS, strlen(cullCS), nullptr, nullptr, nullptr, "cs", "cs_5_0", flags, 0, &pCSBlob, &pErrorBlob);
    if (FAILED(hr)) {
        if (pErrorBlob) OutputDebugStringA((const char*)pErrorBlob->GetBufferPointer());
        assert(false);
    }
    hr = g_pD3DDevice->CreateComputeShader(pCSBlob->GetBufferPointer(), pCSBlob->GetBufferSize(), nullptr, &g_pCullCS);
    assert(SUCCEEDED(hr));
...
void UpdateAABBBuffer()
{
    ...
    g_pD3DContext->UpdateSubresource(g_pCullParamsCB, 0, nullptr, &g_cullParams, 0, 0);
}
...
void ReadQueries()
{
    while (g_lastCompletedFrame < g_curFrame) {
        ...
        HRESULT hr = g_pD3DContext->GetData(g_pQueries[g_lastCompletedFrame % 10], &stats, sizeof(stats), 0);
        if (hr == S_OK) {
            g_gpuVisibleInstances = (int)(stats.IAPrimitives / 12);
...
void UpdateFrustumPlanesCB(const XMMATRIX& vp)
{
    ...
    g_pD3DContext->UpdateSubresource(g_pFrustumPlanesCB, 0, nullptr, planesCPU, 0, 0);
}
...
void CreateInstances()
{
    ...
    for (UINT i = 0; i < MAX_INSTANCES; ++i)
    {
        ...
        float normalMapPresence = (texId == 2) ? 1.0f : 0.0f; // только для like.dds
...
void Render()
{
    ...
    const float kCullingClearTone[4] = { 0.24f, 0.27f, 0.33f, 1.0f };
    g_pD3DContext->ClearRenderTargetView(sceneTarget, kCullingClearTone);
    g_pD3DContext->ClearDepthStencilView(g_pDepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);

    D3D11_VIEWPORT vp = { 0, 0, (float)g_CullingViewWidth, (float)g_CullingViewHeight, 0.0f, 1.0f };
    ...
    float aspect = (float)g_CullingViewWidth / (float)g_CullingViewHeight;
    ...
    g_pD3DContext->CSSetConstantBuffers(0, 2, csCBs);
    ID3D11UnorderedAccessView* csUAVs[] = { g_pIndirectArgsUAVView, g_pVisibleIdsUAV };
    g_pD3DContext->CSSetUnorderedAccessViews(0, 2, csUAVs, nullptr);
    g_pD3DContext->CSSetShader(g_pCullCS, nullptr, 0);

    UINT groupCount = (g_InstanceCount + 63) / 64;
    g_pD3DContext->Dispatch(groupCount, 1, 1);

    ID3D11UnorderedAccessView* nullUAV = nullptr;
    g_pD3DContext->CSSetUnorderedAccessViews(0, 1, &nullUAV, nullptr);
    ...
    g_pD3DContext->VSSetShaderResources(2, 1, &srvVisible);
    g_pD3DContext->PSSetShaderResources(2, 1, &srvVisible);
    ...
    g_pD3DContext->Begin(g_pQueries[g_curFrame % 10]);
    g_pD3DContext->DrawIndexedInstancedIndirect(g_pIndirectArgsDraw, 0);
    g_pD3DContext->End(g_pQueries[g_curFrame % 10]);
    g_curFrame++;
    ...
    if (now - lastTime > 1.0) {
        wchar_t title[256];
        swprintf(title, 256, L"CG AS Culling - Visible: %d", g_gpuVisibleInstances);
        SetWindowTextW(g_hCullingWindow, title);
        lastTime = now;
    }
...
void ResizeWindow(UINT newWidth, UINT newHeight)
{
    ...
    g_CullingViewWidth = newWidth;
    g_CullingViewHeight = newHeight;
